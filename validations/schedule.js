...
validateId: (data) => {
    const { id } = data;
    const findSchedule = prisma.schedule.findUnique({
        data: {where: id}
    });

    if(!id){
        throw new HttpRequestError("id tidak boleh kosong");
    }else if(isNaN(id)){
        throw new HttpRequestError("id harus berupa number");
    }else if(!findSchedule){
        throw new HttpRequestError("id tidak ditemukan");
    }
},
